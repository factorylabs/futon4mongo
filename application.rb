...
class Futon4Mongo < Sinatra::Base
  ...
  get "/_uuids" do 
    ...
    if (db_name && col_name)
      { "uuids" => [Mongo::ObjectID.new.to_s] }.to_json
    else 
      error_not_found
    end
  end
  ...
  get "/:database/:collection/_all_docs" do
    ...
    if startkey_docid
      ...
      if startkey && endkey && params[:limit] == "10" && endkey[-1] == 122
        s = Mongo::ObjectID.from_string(startkey.ljust(24, "0")) rescue startkey
        e = Mongo::ObjectID.from_string(endkey.gsub("z", "f").ljust(24, "f")) rescue endkey
        query["_id"] = {
          "$gte" => s,
          "$lte" => e
        }
  ...
  def getkey(key)
    ...
    if key =~ /^[0-9]+$/
      ...
      return Mongo::ObjectID.from_string(key) rescue key
    end
  end
